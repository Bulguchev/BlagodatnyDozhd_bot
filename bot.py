...
async def show_prayer_times(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id):
    ...
    if not timings:
        await update.message.reply_text("⚠️ *Не удалось загрузить расписание.*", parse_mode='Markdown')
        return
        ...
        try:
                ...
                if prayer_key in timings:
                    ...
                    if current_time == reminder_time:
                        ...
                        await app.bot.send_message(
                            user_id_str,
                            f"⏰ *До намаза {prayer_info['name']} осталось 10 минут!*",
                            parse_mode='Markdown'
                        )
            ...
            for prayer_key in ["Fajr", "Dhuhr", "Asr", "Maghrib", "Isha"]:
                if timings[prayer_key] == current_time:
                    ...
                    await app.bot.send_message(
                        user_id_str,
                        f"{prayer_info['emoji']} *Время намаза {prayer_info['name']}!*",
                        parse_mode='Markdown'
                    )
            ...
            if timings["Fajr"] == current_time:
                ...
                await app.bot.send_message(
                    user_id_str,
                    "🌅 *Не забудьте прочитать утренние азкары!*",
            ...
            if timings["Maghrib"] == current_time:
                ...
                await app.bot.send_message(
                    user_id_str,
                    "🌇 *Не забудьте прочитать вечерние азкары!*",
                    reply_markup=inline_kb,
                    parse_mode='Markdown'
                )
...
async def notification_scheduler(app):
    while True:
