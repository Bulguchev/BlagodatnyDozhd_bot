...
async def get_coordinates_by_city(city_name):
    try:
        async with aiohttp.ClientSession() as session:
            url = f"https://nominatim.openstreetmap.org/search?q={city_name},Russia&format=json&limit=1"
            headers = {'User-Agent': 'MuslimPrayerBot/1.0'}
            async with session.get(url, headers=headers, timeout=10) as response:
                if response.status == 200:
                    data = await response.json()
...
async def get_prayer_times_by_coords(lat, lon):
    try:
        async with aiohttp.ClientSession() as session:
        ...
        logger.error(f"Ошибка получения намазов по координатам: {e}")
    return None

async def get_prayer_times_by_city(city):
    try:
        async with aiohttp.ClientSession() as session:
        ...
        logger.error(f"Ошибка получения намазов по городу: {e}")
    return None
...
async def handle_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if location:
        ...
        if city:
            ...
            times = await get_prayer_times_by_coords(lat, lon)
            ...
            await update.message.reply_text(
                ...
                reply_markup=ReplyKeyboardMarkup([[KeyboardButton("🏙️ Ввести город вручную")]], resize_keyboard=True)
            )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lat, lon = await get_coordinates_by_city(text)
...
async def prayer_times_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in users_db:
        ...
        if user_data['type'] == 'coords' or (user_data['type'] == 'city' and user_data['lat']):
            lat = user_data['lat']
            lon = user_data['lon']
            times = await get_prayer_times_by_coords(lat, lon)
        else:
            times = await get_prayer_times_by_city(city)
    ...
    for user_id, user_data in list(users_db.items()):
        try:
            if user_data['type'] == 'coords' or (user_data['type'] == 'city' and user_data['lat']):
                ...
                times = await get_prayer_times_by_city(user_data['city'])
